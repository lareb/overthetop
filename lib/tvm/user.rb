...
module Tvm
  ...
  class User < Query
    ...
    attr_reader :username, :password, :resp

    def initialize(username, password)
      @username = username
      @password = password
      #raise proper validation error
      ...
      resp = post(sign_in_url, params(username, password))
    ...
    def sign_in_url
      "#{BASE_URL}/api_v3/service/OTTUser/action/login"
    end
    ...
    def params(username, password)
      { "partnerId": PARTNER_Id, "username": username, "password": password, "extra_params": {}, "udid": ""}      
    end
